module Captain::Copilot::ConversationAccess
  ...
  def accessible_conversations(account:, user:)
    # Single chokepoint for every copilot conversation lookup, so hidden
    # conversations stay out of both the search tool and the fetch tool.
    conversations = account.conversations.visible_to_account
