class Rack::Attack
  ...
  # Each throttle's limit is tunable through its own RATE_LIMIT_WIDGET_* override.
  ...
  if ActiveModel::Type::Boolean.new.cast(ENV.fetch('ENABLE_RACK_ATTACK_WIDGET_API', true))
    ...
    throttle('api/v1/widget/conversations', limit: ENV.fetch('RATE_LIMIT_WIDGET_CONVERSATIONS', '30').to_i, period: 1.minute) do |req|
    ...
    throttle('api/v1/widget/messages', limit: ENV.fetch('RATE_LIMIT_WIDGET_MESSAGES', '60').to_i, period: 1.minute) do |req|
    ...
    throttle('api/v1/widget/contact', limit: ENV.fetch('RATE_LIMIT_WIDGET_CONTACTS', '60').to_i, period: 1.hour) do |req|
    ...
    throttle('api/v1/widget/conversations/transcript', limit: ENV.fetch('RATE_LIMIT_WIDGET_TRANSCRIPT', '5').to_i, period: 1.hour) do |req|
