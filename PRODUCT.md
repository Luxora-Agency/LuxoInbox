...
The account-scoped Messenger template API stores reusable fictional scripts, not contact records or real conversation history. It is gated by the default-off Messenger simulator feature: administrators manage scripts, while authorized account agents can read them. Definitions accept contact, agent, custom-attribute and manual variables in message text and timestamps, with a typed contact-avatar choice. At download, agents can fill manual values and optionally upload a local profile photo for that PNG only; neither contact records nor saved templates are changed. A conversation-authorized context endpoint supplies template export data without reading message history and revalidates the selected template and contact snapshot before file delivery.

Administrators manage saved scripts from the simulator page using the same composer and preview as manual simulations; draft replacement and deletion require confirmation. The variable picker inserts literal tokens at the text or timestamp cursor; sample values appear only in previews and sample downloads, never in saved definitions.
