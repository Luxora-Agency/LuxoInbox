class Whatsapp::EmbeddedSignupService
  ...
  def perform
    ...
    # NOTE: We call setup_webhooks explicitly because reauthorization updates an existing channel,
    # so the after_commit on: :create callback does not run. New embedded-signup channels also skip
    # that callback to avoid configuring webhooks twice.
